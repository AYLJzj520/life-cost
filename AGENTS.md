...
这是一个通过 Cloudflare Pages 运行的每日平均固定支出记录工具。用户录入商品名称、购买价格、使用日期和结束日期后，应用按包含起止日期的总使用天数计算单件商品日均成本，并汇总当天实际产生的今日生活成本。
...
- 当商品 `endDate` 早于浏览器当天日期时，自动进入已归档列表，不再计入今日生活成本；结束日期当天仍算作使用中。
- 汇总区显示“今日生活成本”，只统计当天实际产生费用的商品；当天为周末时，勾选“不包含周末”的商品不会计入。
