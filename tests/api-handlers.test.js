...
class FakeD1Statement {
  ...
  async run() {
    ...
    if (this.sql.startsWith("UPDATE items SET end_date = ?")) {
      const hasDailyCost = this.values.length === 6;
      const itemId = hasDailyCost ? this.values[5] : this.values[4];
      ...
      if (hasDailyCost) {
        item.dailyCost = this.values[3];
        item.autoRenew = Boolean(this.values[4]);
      } else {
...
test("PATCH 可同时增加总金额和使用天数并重新计算日均成本", async () => {
  const db = new FakeD1Database([createItem({
    costMode: "total",
    price: 100,
    dailyCost: null,
  ...
  const response = await onRequestPatch({
    ...
    request: new Request("https://example.test/api/items/item-1", {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ price: 150, endDate: "2026-07-15" }),
    }),
    env: { DB: db },
  });
  ...
  assert.equal(item.plannedDays, 15);
  assert.equal(item.price / item.plannedDays, 10);
});

test("PATCH 可修改每日成本并按编辑后的天数重新计算周期总价", async () => {
  const db = new FakeD1Database([createItem()]);
  const response = await onRequestPatch({
    params: { id: "item-1" },
    request: new Request("https://example.test/api/items/item-1", {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ dailyCost: 12, endDate: "2026-07-13" }),
    }),
    env: { DB: db },
  });
  ...
  assert.equal(response.status, 200);
  const { item } = await response.json();
  assert.equal(item.dailyCost, 12);
  assert.equal(item.plannedDays, 6);
  assert.equal(item.price, 72);
});

test("PATCH 拒绝与成本模式不匹配或超出范围的金额字段", async () => {
  const dailyDb = new FakeD1Database([createItem()]);
  const dailyPriceResponse = await onRequestPatch({
    params: { id: "item-1" },
    request: new Request("https://example.test/api/items/item-1", {
      method: "PATCH",
      headers: { "content-type": "application/json" },
  ...
  assert.equal(dailyPriceResponse.status, 400);

  const totalDb = new FakeD1Database([createItem({ costMode: "total", dailyCost: null })]);
  const totalDailyCostResponse = await onRequestPatch({
    params: { id: "item-1" },
    request: new Request("https://example.test/api/items/item-1", {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ dailyCost: 20 }),
    }),
    env: { DB: totalDb },
  });
  assert.equal(totalDailyCostResponse.status, 400);

  const invalidAmountResponse = await onRequestPatch({
    params: { id: "item-1" },
    request: new Request("https://example.test/api/items/item-1", {
      method: "PATCH",
      headers: { "content-type": "application/json" },
