...
商品创建后可以在编辑弹窗中直接修改金额和结束日期：

- 总价分摊商品显示当前总金额，每日固定成本商品显示当前每日成本。
- 调整金额时先输入本次增加或减少的差额，再点击“增加金额”或“减少金额”；减少后的金额必须仍大于 0。
- 结束日期支持手动选择或通过 `-1天` / `+1天` 调整。
- 勾选“不包含周末”的商品按有效使用日增减日期。
- 编辑弹窗实时预览总使用天数、剩余天数和日均成本，保存后服务端重新计算周期天数与价格。
- 延长已归档商品的结束日期后，如果新日期不早于今天，商品会重新进入使用中列表。
